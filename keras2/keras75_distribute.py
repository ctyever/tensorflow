...
#     tf.distribute.ReductionToOneDevice() # 위에거랑 크게 차이는 없음
# )

# strategy = tf.distribute.MirroredStrategy(
#     devices=['/gpu:0'] # 선택적으로 쓸 수 있음
#     # devices=['/gpu:1']
#     # devices=['/cpu','/gpu:1'] # 안 쓰는게 날 수도 있음, gpu를 다 사용하지 않아서 오히려 느려짐
#     # devices=['/cpu','/gpu:0', '/gpu:1'] # cpu 쓰는건 가능하다는것 보여주기 위한 것, 
#     # devices=['/gpu:0', '/gpu:1'] # 이건 불가능 / gpu 두개 같이 쓰려면 위에 strategy 거 사용
# )

# strategy = tf.distribute.experimental.CentralStorageStrategy()
# strategy = tf.distribute.experimental.MultiWorkerMirroredStrategy()
strategy = tf.distribute.experimental.MultiWorkerMirroredStrategy(
    # tf.distribute.experimental.CollectiveCommunication.RING   # 3개가 성능에는 큰 차이는 없음
...
hist = model.fit(x_train, y_train, epochs=3, batch_size=128,  validation_split=0.2, verbose=2)
